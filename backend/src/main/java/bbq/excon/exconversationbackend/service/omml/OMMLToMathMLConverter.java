...
public class OMMLToMathMLConverter {
     ...
     * Note: sup/sub sẽ được xử lý riêng trong
     * convertSuperscriptWithContext/convertSubscriptWithContext
    ...
    private static String convertNaryOperator(OMMLNode naryNode) {
        ...
        if (!lowerLimit.isEmpty() || !upperLimit.isEmpty()) {
            // With limits:
            // <munderover><mo>operator</mo><munder>lower</munder><mover>upper</mover></munderover>
     ...
     * Ví dụ: "y=ax-4bx+c" ->
     * <mi>y</mi><mo>=</mo><mi>a</mi><mi>x</mi><mo>-</mo><mn>4</mn>...
     */
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (isOperatorChar(c) && !(c == '-' && (i == 0 || expectOperator || currentToken.length() == 0))) {
                ...
                expectOperator = true;
                    ...
                    || (c == '-' && (i == 0 || expectOperator || currentToken.length() == 0))) {
